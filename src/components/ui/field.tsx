...
type FieldShellProps = { id: string; label: string; error?: string; hint?: string; labelClassName?: string; children: ReactNode }

export function FieldShell({ id, label, error, hint, labelClassName, children }: FieldShellProps) {
  return <div className="grid gap-2"><label id={`${id}-label`} htmlFor={id} className={cn('text-sm font-extrabold tracking-[-.01em] text-ink', labelClassName)}>{label}</label>{children}{hint && !error && <p id={`${id}-hint`} className="text-xs leading-relaxed text-muted">{hint}</p>}{error && <p id={`${id}-error`} role="alert" className="text-xs font-bold text-error">{error}</p>}</div>
}
