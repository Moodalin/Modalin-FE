...
import { ArrowLeft, ArrowRight, CalendarDays, ChevronDown, Clock3, MapPin, ShieldCheck, Users } from 'lucide-react'
...
export default function CampaignPublicPage() {
  ...
  return <div className="bg-white"><MarketingHeader /><main id="main-content" className="pb-24 lg:pb-0">
    <section className="landing-soft-gradient border-b border-line px-5 py-8 lg:px-8 lg:py-12">
      <div className="mx-auto max-w-[1180px]"><ButtonLink to="/campaign" variant="ghost" className="mb-6 px-0"><ArrowLeft size={17} aria-hidden="true" />Kembali ke koleksi</ButtonLink></div>
      <div className="mx-auto grid max-w-[1180px] gap-9 lg:grid-cols-[1.02fr_.98fr] lg:items-start lg:gap-14">
        ...
        <div className="flex min-w-0 flex-col">
          ...
          {campaign.acceptsOrders ? <ButtonLink to={`/checkout?campaign=${campaign.id}`} className="mt-4 w-full">Pesan produk <ArrowRight size={17} aria-hidden="true" /></ButtonLink> : <div className="mt-4 rounded-xl border border-line bg-cream px-4 py-3 text-center text-sm font-bold text-muted">Pre-order sudah ditutup.</div>}
          <p className="mt-3 text-center text-xs leading-5 text-muted">{campaign.status === 'in_production' && campaign.acceptsOrders ? 'Produksi berjalan. Pesanan tambahan tetap diterima sampai batas waktu.' : 'Produksi dimulai setelah target minimum tercapai.'}</p>
        </div>
      </div>
    </section>
    <section className="bg-white px-5 py-16 lg:px-8 lg:py-20"><div className="mx-auto max-w-[1180px]"><SectionHeading eyebrow="Koleksi" title="Pilih produk untuk dipesan." description="Pilih produk dan varian yang tersedia. Perbedaan kecil pada warna dan tekstur merupakan hasil proses buatan tangan." /><div className="mt-9 grid gap-5 lg:grid-cols-2">{campaign.products.map((product) => <ProductCard key={product.id} product={product} campaignId={campaign.id} orderingOpen={campaign.acceptsOrders} />)}</div></div></section>
  ...
  </main>{campaign.acceptsOrders && <div className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-white p-3 lg:hidden"><ButtonLink to={`/checkout?campaign=${campaign.id}`} className="w-full">Pesan produk · {Math.round(progress)}%</ButtonLink></div>}<MarketingFooter /></div>
}
